...
const targetPath = path.join(repoRoot, 'src/config/testModeFixtures.ts');

const nextContent = `export type TestModeUserType = 'studio' | 'pro';
...
async function run() {
  ...
  if (currentContent === nextContent) {
    console.log('[seed:test] Fixtures déjà à jour.');
    return;
  }

  await writeFile(targetPath, nextContent, 'utf8');
  console.log(`[seed:test] Fixtures écrites dans ${path.relative(repoRoot, targetPath)}.`);
}

run().catch((error) => {
  console.error('[seed:test] Échec:', error);
  process.exitCode = 1;
});
