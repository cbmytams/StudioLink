...
export function MissionCard({ mission, hasApplied = false, to }: MissionCardProps) {
  ...
  return (
    <Link
      to={to}
      className="mission-card app-card-soft block h-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-300"
    ...
    >
      <div className="flex h-full flex-col gap-4 p-5 text-left">
        <div className="flex items-start justify-between gap-3">
          ...
          {mission.genre ? (
            <span className="shrink-0 rounded-full border border-orange-300/40 bg-orange-400/10 px-2.5 py-1 text-[var(--text-2xs-plus)] font-semibold text-orange-200">
              {mission.genre}
            </span>
          ) : null}
        </div>
