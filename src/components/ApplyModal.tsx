...
export function ApplyModal({ isOpen, missionId, onClose, onSubmitted }: ApplyModalProps) {
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-modal flex items-end justify-center bg-[var(--color-surface-soft)]/85 p-0 backdrop-blur-sm sm:items-center sm:p-4"
      onMouseDown={(event) => {
    ...
    >
      <FocusTrap>
        <div
          className="flex max-h-[var(--size-modal-max-dvh)] w-full flex-col overflow-y-auto rounded-t-3xl border border-white/60 bg-white/95 shadow-[var(--shadow-overlay-sm)] sm:max-w-md sm:rounded-2xl"
        ...
        >
          ...
          <div className="sticky top-0 z-10 flex items-center justify-between border-b border-black/5 bg-white/90 px-5 py-4 backdrop-blur-sm">
            ...
            <button
              ...
              aria-label="Fermer"
              onClick={onClose}
              className="flex min-h-[var(--size-touch)] min-w-[var(--size-touch)] items-center justify-center rounded-full text-stone-500 transition hover:bg-stone-100 hover:text-stone-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-300 focus-visible:ring-offset-2"
            >
              <X size={18} />
            </button>
          </div>
