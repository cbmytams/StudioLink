...
import { setOnboardingProgress, updateProfileIdentity, upsertStudioProfile } from '@/services/profileService';
...
export default function StudioOnboarding() {
  ...
  }, [navigate, profile, setCurrentStep, step]);
  ...
  const handleNext = () => {
    ...
    setErrors({});
    setDirection(1);
    const nextStepValue = Math.min(3, step + 1);
    if (session?.user?.id) {
      void setOnboardingProgress(session.user.id, nextStepValue, false);
    }
    nextStep();
  };
  ...
  const handlePrev = () => {
    ...
    if (session?.user?.id) {
      void setOnboardingProgress(session.user.id, previousStepValue, false);
    }
    prevStep();
  };
  ...
  const handleComplete = async () => {
    if (session?.user?.id) {
      await upsertStudioProfile({
        ...
        district: studioData.district || null,
      ...
      await updateProfileIdentity(session.user.id, { display_name: studioData.name || null });
      await setOnboardingProgress(session.user.id, 3, true);
  ...
  const handleSkip = async () => {
    if (session?.user?.id) {
      await setOnboardingProgress(session.user.id, step, true);
      await refreshProfile();
    }
    setCurrentStep(1);
    navigate('/studio/dashboard');
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <select 
                      ...
                      className="w-full rounded-lg border border-stone-200 bg-white/80 px-4 py-3 text-sm text-stone-800 placeholder:text-stone-400 focus:border-orange-400 focus:outline-none focus:ring-1 focus:ring-orange-400 transition-colors duration-150"
                    >
                      {DISTRICTS.map(d => (
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full rounded-lg border border-stone-200 bg-white/80 pl-12 pr-4 py-3 text-sm text-stone-800 placeholder:text-stone-400 focus:border-orange-400 focus:outline-none focus:ring-1 focus:ring-orange-400 transition-colors duration-150"
                      />
                    </div>
                  </div>
                </div>
