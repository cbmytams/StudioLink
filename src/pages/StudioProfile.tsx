  ...
  return (
    <div className="min-h-screen bg-[#f4ece4] pb-32">
      ...
      <div className="pt-14 relative h-64 w-full overflow-hidden bg-black/5">
        <div className="flex overflow-x-auto snap-x snap-mandatory h-full no-scrollbar">
          {[1, 2, 3].map((i) => (
            <div key={i} className="w-full h-full shrink-0 snap-center relative">
              ...
              />
              {isEditing && (
                <button className="absolute top-4 right-4 w-8 h-8 bg-black/50 backdrop-blur-md rounded-full flex items-center justify-center text-white hover:bg-red-500 transition-colors">
                  <X size={16} />
                </button>
              )}
            </div>
        ...
        {isEditing && (
          <button className="absolute bottom-4 right-4 w-10 h-10 bg-orange-500 rounded-full flex items-center justify-center text-white shadow-lg hover:bg-orange-600 transition-colors">
            <Plus size={20} />
          </button>
        )}
      </div>
        ...
        <GlassCard className="p-6 flex flex-col gap-4">
          {isEditing ? (
            <TextInput 
              ...
              className="text-2xl font-semibold bg-transparent border-b border-black/10 rounded-none px-0 focus:border-orange-500"
            />
          ) : (
